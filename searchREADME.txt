...
2. To get the filtered and formatted data, run search.py:
 	...
 	the third argument as the search term you are looking for. If the search term is a hashtag (e.g. #BlackLivesMatter) do not include the hashtag (compilers have trouble dealing with special characters), if it is a username or plain text just leave it as such. If this term includes spaces, every space must have a backslash immediately preceding it.
 	...
 	the fourth argument informs the program whether or not the search term is a hashtag, plain text in the tweet, or a username . If the search term is a hashtag or in the text, then the fourth argument should be 'text'. If the search term is a username then this argument should be 'username' 

 	OPTIONAL ARGUMENTS 

 	These additional arguments will end up being the differently colored nodes in the final visualization.

	arguments five through eight are additional optional search terms to look for within the targeted tweets, along with whether or not they are hashtags or plain text. (e.g. looking for '#ICantBreathe', and '#Ferguson' within tweets that already contain '#BlackLivesMatter'). 



	arguments five through seven are additional search terms (without any preceding hashtags).

	

	Argument eight is whether these additional search terms are the twitter handle, 'username' or hashtag/plain text, 'text'. 
...
If I wanted to look at tweets by user 'original_tweeter' for the first two weeks of a month, while changing node colors for tweets that also contain the tweets '#AllLivesMatter' and 'hello world' I would run:

	'python search.py 1 15 original_tweeter username AllLivesMatter hello\ world text'

And if I wanted to look at '#BlackLivesMatter' tweets for the first eight days of the month while changing node colors for tweets that also contain '#AllLivesMatter,' '#ICantBreathe,' and '#HandsUpDontShoot' I would run:

	'python search.py 1 9 \#BlackLivesMatter text AllLivesMatter ICantBreathe HandsUpDontShoot text'
