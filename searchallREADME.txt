...
the allsearch.py script is designed to create a network visualization of multiple search terms in twitter data. It creates one to multicolored nodes based on the extant hashtags (or search terms) in a user's twitter data.

PART 1:
Part just unzip the files as you did in the searchREADME.txt.
...
2. To get the filtered and formatted data, run search.py:
 	...
 	the third through second to last arguments are the search terms you are looking for. If the search terms are hashtags (e.g. #BlackLivesMatter) do not include the hashtag (compilers have trouble dealing with special characters), if they are  usernames or plain text just leave it as such. If the terms include spaces, every space must have a backslash immediately preceding it.
 	...
 	the final argument informs the program whether or not the search term is a hashtag, plain text in the tweet, or a username . If the search term is a hashtag or in the text, then the fourth argument should be 'hashtag' or 'text'. If the search term is a username then this argument should be 'username' 
...
If I wanted to look at tweets that contained '#AllLivesMatter' and '#BlackLivesMatter' in the first two weeks of a month I would run:

	'python search.py 1 15  \#BlackLivesMatter \#AllLivesMatter text'
...
Part 3 – Create Visualization
	...
	2. After opening this file, go to line 89 (in TextEdit you can do this by hitting command+l and entering the line number) and change the filepath from "formatted_data/formatted.json" to "formatted_data/" + the name of the file you just created in Part 2. (e.g I would make this new filepath "formatted_data/formatted1-2-#BlackLivesMatter.json")
	...
	4. Go to your favorite web browser and navigate to 'localhost:8000/all_vis.html', replacing the 8000 with the port that you are running on and the twitter.html with what you named your new .html file. 

	5. Tweak it! To tweak the size of the enclosing box, size of the nodes, and distance between the nodes, go to lines 48-55 in the .html file. You will something like this:
